  ...
  if (isErrorMyReports) {
    return (
      <Text className='text-red-500'>
        Error loading reports: {errorMyReports?.message ?? 'An unknown error occurred'}
      </Text>
    );
  }
    ...
    <View className='flex-1 px-8 pt-4 bg-[#FBFDFC]'>
      <View className='flex flex-col gap-y-4'>
        <FlatList
          ...
          renderItem={({ item }) => <ReportsCardComponent
            id={item.id}
            onPress={() => router.push(`/reports/${item.id}`)}
            imageCid={{ uri: `https://gateway.ipfs.io/ipfs/${item.imageCid}` }}
            userProfile={{ uri: `https://gateway.ipfs.io/ipfs/${item.userProfile}` }}
