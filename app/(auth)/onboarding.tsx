  ...
  return (
    <AuthLayout>
      ...
      <Swiper
        ...
        loop={false}
        showsButtons={false}
        dot={<View className='mx-1 h-[8px] w-[8px] rounded-full bg-neutral-400'></View>}
        ...
        {onboarding.map((item, index) => (
          <View key={index} className='flex justify-center items-center p-1'>
            ...
            <Text className='font-Regular text-lg text-center'>{item.description}</Text>
          </View>
        ))}
      </Swiper>
      <CustomButton title={isLastSlide ? 'Get Started' : 'Next'} bgVariant='primary' textVariant='secondary' onPress={() => isLastSlide ? router.replace('/(auth)/sign-in') : swiperRef.current?.scrollBy(1)} className="mt-10 mb-12 w-11/12" IconLeft={() => isLastSlide ? <Ionicons name='rocket-outline' size={24} color='white' className='mr-2' /> : null} />
    </AuthLayout>
  )
}
