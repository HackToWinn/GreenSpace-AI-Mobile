...
export default function Statistics() {
  ...
  const categoryData: CategoryData[] = [
    {
      title: 'Air Pollution',
      value: 30,
      chartData: [{ value: 5 }, { value: 3 }, { value: 4 }, { value: 6 }, { value: 12 }],
      changes: '+5'
    },
    {
      title: 'Water Pollution',
      value: 15,
      chartData: [{ value: 2 }, { value: 3 }, { value: 4 }, { value: 5 }, { value: 1 }],
      changes: '-2'
    },
    {
      title: 'Deforestation',
      value: 25,
      chartData: [{ value: 6 }, { value: 5 }, { value: 7 }, { value: 4 }, { value: 3 }],
      changes: '+3'
    },
    {
      title: 'Waste Management',
      value: 20,
      chartData: [{ value: 4 }, { value: 5 }, { value: 6 }, { value: 3 }, { value: 2 }],
      changes: '+1'
    }
  ];

  const trendsData: { [range: string]: TrendData[] } = {
    daily: [{ value: 5 }, { value: 3 }, { value: 2 }, { value: 1 }, { value: 1 }],
    weekly: [{ value: 4 }, { value: 3 }, { value: 2 }, { value: 1 }, { value: 4 }],
    monthly: [{ value: 8 }, { value: 6 }, { value: 4 }, { value: 1 }, { value: 5 }]
  };
  ...
  return (
    <Layout className="pt-4 h-full">
      ...
      {/* Konten scrollable */}
      <ScrollView contentContainerStyle={{ paddingTop: 4 }} scrollEventThrottle={16} className="h-full" decelerationRate="normal" showsVerticalScrollIndicator={false} showsHorizontalScrollIndicator={false}>
        <View id="trends">
          <View>
            <View className="px-8">
              ...
              <View className="flex-row items-center mb-4">
                <View className="bg-green-100 px-2 py-1 rounded-lg flex-row items-center">
                  <Text className="text-green-600 font-semibold text-sm mr-1">⬆</Text>
                  <Text className="text-green-600 font-SemiBold text-sm">5% Today</Text>
                </View>
              </View>
            ...
            <View
              ...
              <Animated.View
                ...
                <LineChart data={getTrendChartData()} thickness={3} color={getTrendChartColor()} hideDataPoints={true} dataPointsRadius={4} yAxisColor="transparent" xAxisColor="transparent" curved animationDuration={800} height={220} initialSpacing={20} spacing={100} rulesColor="#E5E7EB" isAnimated={true} curveType={CurveType.QUADRATIC} hideRules hideYAxisText animateOnDataChange={true} />
              </Animated.View>
            </View>
            ...
            <View className="px-8">
              ...
              <View className="flex-row flex-wrap justify-between">
                {categoryData.map((item, index) => (
                  <View key={index} className={`w-[48%] rounded-xl p-4 mb-3  bg-green-100/50`}>
                    ...
                    <View
                      ...
                      <LineChart data={item.chartData} thickness={2} color="#10B981" hideDataPoints={true} yAxisColor="transparent" xAxisColor="transparent" curved animationDuration={800} height={60} initialSpacing={20} spacing={30} rulesColor="#E5E7EB" isAnimated={true} curveType={CurveType.QUADRATIC} hideRules hideYAxisText animateOnDataChange={true} />
                    </View>
                    <Text className={`font-Regular ${item.changes.includes('+') ? 'text-green-500' : 'text-red-500'}`}>{item.changes}</Text>
                  </View>
                ))}
              </View>
            </View>
          ...
          <View className="mb-6 rounded-2xl shadow-sm shadow-neutral-400 overflow-hidden">
            <View className="p-4">
              <View className="flex-row justify-between items-center mb-4">
                <Text className="text-lg font-Bold text-gray-800">{showDetail ? getChartTitle() : `Statistics: ${selectedYear}`}</Text>
                {!showDetail && (
                  <View className="bg-red-100 px-3 py-1 rounded-full">
                    <Text className="text-red-600 text-sm font-Medium">{getCurrentYearData().find(item => item.label === selectedDay)?.value || 0} Accidents</Text>
                  </View>
                )}
                <View className="flex-row items-center">
                  ...
                  <TouchableOpacity className={`px-4 py-2 rounded-lg ${showDetail ? 'bg-red-600' : 'bg-green-600'}`} onPress={toggleDetail}>
                    <Text className="text-white font-Medium">{showDetail ? 'Kembali' : 'Detail'}</Text>
                  </TouchableOpacity>
                </View>
              </View>
              ...
              <View className="mb-4">
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 4 }}>
                  {availableYears.map(year => (
                    <TouchableOpacity key={year} className={`px-4 py-2 rounded-full mr-2 ${selectedYear === year ? 'bg-green-600' : 'bg-gray-200'}`} onPress={() => handleYearChange(year as Year)}>
                      <Text className={`font-Medium ${selectedYear === year ? 'text-white' : 'text-gray-700'}`}>{year}</Text>
                    </TouchableOpacity>
                  ))}
                </ScrollView>
              </View>
                ...
                <View className="mb-4">
                  <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 4 }}>
                    ...
                    {getCurrentYearData().map((item, index) => (
                      <TouchableOpacity key={index} className={`px-4 py-2 rounded-full mr-2 ${selectedDay === item.label ? 'bg-red-600' : 'bg-gray-200'}`} onPress={() => handleDayChange(item.label as Day)}>
                        <Text className={`font-Medium ${selectedDay === item.label ? 'text-white' : 'text-gray-700'}`}>{monthNames[item.label]}</Text>
                      </TouchableOpacity>
                    ))}
                  </ScrollView>
                </View>
