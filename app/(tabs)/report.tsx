...
import { Text, View, FlatList, Dimensions, ScrollView, ActivityIndicator } from 'react-native';
  ...
  if (isErrorLatest || isErrorMyReports) {
    return (
      <Text className='text-red-500'>
        Error loading reports: {(errorLatest?.message ?? 'An unknown error occurred') || (errorMyReports?.message ?? 'An unknown error occurred')}
      </Text>
    );
  }
        ...
        <View className='flex flex-col items-start gap-y-8'>
          ...
          <View className='flex flex-col gap-y-4'>
            ...
            <FlatList
              ...
              renderItem={({ item }) => <ReportsPageCardComponent
                id={item.id}
                onPress={() => router.push(`/reports/${item.id}`)}
                imageCid={{ uri: `https://gateway.ipfs.io/ipfs/${item.imageCid}` }}
                userProfile={{ uri: `https://gateway.ipfs.io/ipfs/${item.user.pictureCid}` }}
